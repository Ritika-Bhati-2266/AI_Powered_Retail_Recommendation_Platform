...
export default function ProductSearch({ showAllOnMount, customerId, externalQuery, onExternalQueryChange, onAddToCart, onWishlist, onProductClick }: {
  ...
  const doSearch = useCallback(async (q: string, cat: string) => {
    ...
    try {
      ...
      // The backend may be down/restarting — show a recoverable error instead
      // of pretending there are no matching products.
      setResults([]);
      setSearchError(true);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-5">
      ...
      {!loading && searchError && (
        <div className="bg-zinc-900/50 border border-zinc-800/50 rounded-2xl p-12 text-center space-y-4">
          <AlertCircle className="w-12 h-12 text-amber-500/70 mx-auto" />
          <div>
            <h3 className="text-base font-semibold text-zinc-200">Search is temporarily unavailable</h3>
            <p className="text-sm text-zinc-500 mt-1">
              The product catalogue could not be loaded. The server may be starting up or busy. Try again in a moment.
            </p>
          </div>
          <button
            onClick={() => doSearch(query.trim(), category)}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-purple-500/10 hover:bg-purple-500/20 text-purple-300 text-sm font-medium transition-all"
          >
